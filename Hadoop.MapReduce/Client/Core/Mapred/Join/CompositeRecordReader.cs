...
namespace Org.Apache.Hadoop.Mapred.Join
{
	...
	/// </summary>
	public abstract class CompositeRecordReader<K, V, X> : Configurable
		...
		where X : Writable
	{
		...
		private sealed class _IComparer_78 : IComparer<ComposableRecordReader<K, object>>
		{
		...
		private sealed class _IComparer_136 : IComparer<ComposableRecordReader<K, object>
			>
		{
			public _IComparer_136(CompositeRecordReader<K, V, X> _enclosing)
			{
				this._enclosing = _enclosing;
			}
		...
		internal class JoinCollector
		{
			...
			protected internal virtual bool HasNext()
			{
				return !(this.pos < 0);
			}
		...
		/// <summary>Create a new key value common to all child RRs.</summary>
		...
		public virtual K CreateKey()
		{
			...
			if (null == keyclass)
			{
				Type cls = kids[0].CreateKey().GetType();
				foreach (RecordReader<K, Writable> rr in kids)
				{
					if (!cls.Equals(rr.CreateKey().GetType()))
					{
						throw new InvalidCastException("Child key classes fail to agree");
					}
				}
				keyclass = cls.AsSubclass<WritableComparable>();
			}
		...
		/// <summary>Unsupported (returns zero in all cases).</summary>
		/// <exception cref="System.IO.IOException"/>
