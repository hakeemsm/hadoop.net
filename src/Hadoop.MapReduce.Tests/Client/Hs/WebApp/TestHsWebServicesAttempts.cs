...
namespace Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp
{
	...
	/// counters
	/// </summary>
	public class TestHsWebServicesAttempts : JerseyTest
	{
		...
		private sealed class _ServletModule_88 : ServletModule
		{
			...
			{
				Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesAttempts.appContext = new 
					MockHistoryContext(0, 1, 2, 1);
				Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesAttempts.webApp = Org.Mockito.Mockito
					.Mock<HsWebApp>();
				Org.Mockito.Mockito.When(Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesAttempts
					.webApp.Name()).ThenReturn("hsmockwebapp");
				this.Bind<JAXBContextResolver>();
				this.Bind<HsWebServices>();
				this.Bind<GenericExceptionHandler>();
				this.Bind<WebApp>().ToInstance(Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesAttempts
					.webApp);
				this.Bind<AppContext>().ToInstance(Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesAttempts
					.appContext);
				this.Bind<HistoryContext>().ToInstance(Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesAttempts
					.appContext);
				this.Bind<Configuration>().ToInstance(Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesAttempts
					.conf);
		...
		public class GuiceServletConfig : GuiceServletContextListener
		{
			...
			internal GuiceServletConfig(TestHsWebServicesAttempts _enclosing)
			{
				this._enclosing = _enclosing;
			}

			private readonly TestHsWebServicesAttempts _enclosing;
		}
		...
		public TestHsWebServicesAttempts()
			: base(new WebAppDescriptor.Builder("org.apache.hadoop.mapreduce.v2.hs.webapp").ContextListenerClass
				(typeof(TestHsWebServicesAttempts.GuiceServletConfig)).FilterClass(typeof(GuiceFilter
			...
			foreach (JobId id in jobsMap.Keys)
			{
				...
				foreach (Task task in jobsMap[id].GetTasks().Values)
				{
					...
					ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
						).Path("jobs").Path(jobId).Path("tasks").Path(tid).Path("attempts").Accept(MediaType
		...
		public virtual void TestTaskAttemptsDefault()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				...
				foreach (Task task in jobsMap[id].GetTasks().Values)
				{
					string tid = MRApps.ToString(task.GetID());
					ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
						).Path("jobs").Path(jobId).Path("tasks").Path(tid).Path("attempts").Get<ClientResponse
						>();
					NUnit.Framework.Assert.AreEqual(MediaType.ApplicationJsonType, response.GetType()
						);
					JSONObject json = response.GetEntity<JSONObject>();
		...
		{
			TestTaskAttemptIdErrorGeneric("bogusid", "java.lang.Exception: TaskAttemptId string : "
		...
		{
			TestTaskAttemptIdErrorGeneric("attempt_0_1234_d_000000_0", "java.lang.Exception: Bad TaskType identifier. TaskAttemptId string : "
		...
		{
			TestTaskAttemptIdErrorGeneric("attempt_1234_m_000000_0", "java.lang.Exception: TaskAttemptId string : "
		...
		{
			TestTaskAttemptIdErrorGeneric("attempt_0_1234_m_000000", "java.lang.Exception: TaskAttemptId string : "
		...
		public virtual void VerifyHsTaskAttemptsXML(NodeList nodes, Task task)
		{
