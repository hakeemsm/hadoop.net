...
{
	public class RMDelegationTokenSelector : TokenSelector<RMDelegationTokenIdentifier
		>
	{
		private static readonly Log Log = LogFactory.GetLog(typeof(RMDelegationTokenSelector
			));
		...
		public virtual Org.Apache.Hadoop.Security.Token.Token<RMDelegationTokenIdentifier
			...
			>> tokens)
		{
			...
			foreach (Org.Apache.Hadoop.Security.Token.Token<TokenIdentifier> token in tokens)
			{
				Log.Debug("Token kind is " + token.GetKind().ToString() + " and the token's service name is "
					 + token.GetService());
				if (RMDelegationTokenIdentifier.KindName.Equals(token.GetKind()) && CheckService(
					service, token))
				{
