...
{
	public class TestHTestCase : HTestCase
	{
		...
		{
			long start = Time.Now();
			long waited = WaitFor(1000, new _Predicate_66());
			long end = Time.Now();
			NUnit.Framework.Assert.AreEqual(waited, 0, 50);
			NUnit.Framework.Assert.AreEqual(end - start - waited, 0, 50);
		}

		private sealed class _Predicate_66 : HTestCase.Predicate
		{
			public _Predicate_66()
			{
			}
		...
		public virtual void WaitForTimeOutRatio1()
		{
			SetWaitForRatio(1);
			long start = Time.Now();
			long waited = WaitFor(200, new _Predicate_81());
			long end = Time.Now();
		...
		public virtual void TestJetty()
		{
			...
			context.AddServlet(typeof(TestHTestCase.MyServlet), "/bar");
