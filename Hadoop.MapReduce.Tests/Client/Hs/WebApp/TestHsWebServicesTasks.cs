...
namespace Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp
{
	...
	/// </remarks>
	public class TestHsWebServicesTasks : JerseyTest
	{
		...
		private sealed class _ServletModule_84 : ServletModule
		{
			public _ServletModule_84()
			{
			}

			protected override void ConfigureServlets()
			{
				Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesTasks.appContext = new 
					MockHistoryContext(0, 1, 2, 1);
				Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesTasks.webApp = Org.Mockito.Mockito
					.Mock<HsWebApp>();
				Org.Mockito.Mockito.When(Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesTasks
					.webApp.Name()).ThenReturn("hsmockwebapp");
				this.Bind<JAXBContextResolver>();
				this.Bind<HsWebServices>();
				this.Bind<GenericExceptionHandler>();
				this.Bind<WebApp>().ToInstance(Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesTasks
					.webApp);
				this.Bind<AppContext>().ToInstance(Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesTasks
					.appContext);
				this.Bind<HistoryContext>().ToInstance(Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesTasks
					.appContext);
				this.Bind<Configuration>().ToInstance(Org.Apache.Hadoop.Mapreduce.V2.HS.Webapp.TestHsWebServicesTasks
					.conf);
				this.Serve("/*").With(typeof(GuiceContainer));
			}
		}

		private Injector injector = Guice.CreateInjector(new _ServletModule_84());
		...
		public class GuiceServletConfig : GuiceServletContextListener
		{
			...
			internal GuiceServletConfig(TestHsWebServicesTasks _enclosing)
			{
				this._enclosing = _enclosing;
			}

			private readonly TestHsWebServicesTasks _enclosing;
		}
		...
		public TestHsWebServicesTasks()
			: base(new WebAppDescriptor.Builder("org.apache.hadoop.mapreduce.v2.hs.webapp").ContextListenerClass
				(typeof(TestHsWebServicesTasks.GuiceServletConfig)).FilterClass(typeof(GuiceFilter
		...
		public virtual void TestTasks()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				...
				ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
					).Path("jobs").Path(jobId).Path("tasks").Accept(MediaType.ApplicationJson).Get<ClientResponse
					>();
				...
				VerifyHsTask(arr, jobsMap[id], null);
			}
		}
		...
		public virtual void TestTasksDefault()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				string jobId = MRApps.ToString(id);
				ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
					).Path("jobs").Path(jobId).Path("tasks").Get<ClientResponse>();
				NUnit.Framework.Assert.AreEqual(MediaType.ApplicationJsonType, response.GetType()
					);
				JSONObject json = response.GetEntity<JSONObject>();
				NUnit.Framework.Assert.AreEqual("incorrect number of elements", 1, json.Length());
		...
		public virtual void TestTasksSlash()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				string jobId = MRApps.ToString(id);
				ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
					).Path("jobs").Path(jobId).Path("tasks/").Accept(MediaType.ApplicationJson).Get<
					ClientResponse>();
				NUnit.Framework.Assert.AreEqual(MediaType.ApplicationJsonType, response.GetType()
					);
				JSONObject json = response.GetEntity<JSONObject>();
		...
		public virtual void TestTasksXML()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				string jobId = MRApps.ToString(id);
				ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
					).Path("jobs").Path(jobId).Path("tasks").Accept(MediaType.ApplicationXml).Get<ClientResponse
					>();
				...
				VerifyHsTaskXML(task, jobsMap[id]);
			}
		}
		...
		public virtual void TestTasksQueryMap()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				...
				string type = "m";
				ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
					).Path("jobs").Path(jobId).Path("tasks").QueryParam("type", type).Accept(MediaType
				...
				VerifyHsTask(arr, jobsMap[id], type);
			}
		}
			...
			foreach (JobId id in jobsMap.Keys)
			{
				...
				try
				{
					r.Path("ws").Path("v1").Path("history").Path("mapreduce").Path("jobs").Path(jobId
						).Path("tasks").QueryParam("type", tasktype).Accept(MediaType.ApplicationJson).Get
						<JSONObject>();
		...
		public virtual void TestTaskId()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				...
				foreach (Task task in jobsMap[id].GetTasks().Values)
				{
					...
					ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
						).Path("jobs").Path(jobId).Path("tasks").Path(tid).Accept(MediaType.ApplicationJson
						).Get<ClientResponse>();
					NUnit.Framework.Assert.AreEqual(MediaType.ApplicationJsonType, response.GetType()
						);
					JSONObject json = response.GetEntity<JSONObject>();
		...
		public virtual void TestTaskIdDefault()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				...
				foreach (Task task in jobsMap[id].GetTasks().Values)
				{
					string tid = MRApps.ToString(task.GetID());
					ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
						).Path("jobs").Path(jobId).Path("tasks").Path(tid).Get<ClientResponse>();
					NUnit.Framework.Assert.AreEqual(MediaType.ApplicationJsonType, response.GetType()
						);
					JSONObject json = response.GetEntity<JSONObject>();
					NUnit.Framework.Assert.AreEqual("incorrect number of elements", 1, json.Length());
		...
		public virtual void TestTaskIdBogus()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				...
				{
					r.Path("ws").Path("v1").Path("history").Path("mapreduce").Path("jobs").Path(jobId
						).Path("tasks").Path(tid).Get<JSONObject>();
					NUnit.Framework.Assert.Fail("should have thrown exception on invalid uri");
				}
				catch (UniformInterfaceException ue)
				{
		...
		public virtual void TestTaskIdXML()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				...
				foreach (Task task in jobsMap[id].GetTasks().Values)
				{
					string tid = MRApps.ToString(task.GetID());
					ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
						).Path("jobs").Path(jobId).Path("tasks").Path(tid).Accept(MediaType.ApplicationXml
						).Get<ClientResponse>();
					NUnit.Framework.Assert.AreEqual(MediaType.ApplicationXmlType, response.GetType());
					string xml = response.GetEntity<string>();
					DocumentBuilderFactory dbf = DocumentBuilderFactory.NewInstance();
		...
		/// <exception cref="Org.Codehaus.Jettison.Json.JSONException"/>
		public virtual void VerifyHsSingleTask(JSONObject info, Task task)
		{
		...
		/// <exception cref="Org.Codehaus.Jettison.Json.JSONException"/>
		public virtual void VerifyHsTask(JSONArray arr, Org.Apache.Hadoop.Mapreduce.V2.App.Job.Job
			 job, string type)
		{
		...
		public virtual void VerifyHsSingleTaskXML(Element element, Task task)
		{
			VerifyTaskGeneric(task, WebServicesTestUtils.GetXmlString(element, "id"), WebServicesTestUtils
				...
				.GetXmlFloat(element, "progress"));
		}

		public virtual void VerifyHsTaskXML(NodeList nodes, Org.Apache.Hadoop.Mapreduce.V2.App.Job.Job
			 job)
		{
		...
		public virtual void TestTaskIdCounters()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				...
				foreach (Task task in jobsMap[id].GetTasks().Values)
				{
					string tid = MRApps.ToString(task.GetID());
					ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
						).Path("jobs").Path(jobId).Path("tasks").Path(tid).Path("counters").Accept(MediaType
						.ApplicationJson).Get<ClientResponse>();
					NUnit.Framework.Assert.AreEqual(MediaType.ApplicationJsonType, response.GetType()
						);
					JSONObject json = response.GetEntity<JSONObject>();
		...
		public virtual void TestTaskIdCountersDefault()
		{
			...
			foreach (JobId id in jobsMap.Keys)
			{
				...
				foreach (Task task in jobsMap[id].GetTasks().Values)
				{
					string tid = MRApps.ToString(task.GetID());
					ClientResponse response = r.Path("ws").Path("v1").Path("history").Path("mapreduce"
						).Path("jobs").Path(jobId).Path("tasks").Path(tid).Path("counters").Get<ClientResponse
						>();
					NUnit.Framework.Assert.AreEqual(MediaType.ApplicationJsonType, response.GetType()
						);
					JSONObject json = response.GetEntity<JSONObject>();
		...
		/// <exception cref="Org.Codehaus.Jettison.Json.JSONException"/>
		public virtual void VerifyHsJobTaskCounters(JSONObject info, Task task)
		{
		...
		public virtual void VerifyHsTaskCountersXML(NodeList nodes, Task task)
		{
			for (int i = 0; i < nodes.GetLength(); i++)
			{
				Element element = (Element)nodes.Item(i);
