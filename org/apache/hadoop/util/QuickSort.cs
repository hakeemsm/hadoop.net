...
namespace org.apache.hadoop.util
{
	...
	public sealed class QuickSort : org.apache.hadoop.util.IndexedSorter
	{
		private static readonly org.apache.hadoop.util.IndexedSorter alt = new org.apache.hadoop.util.HeapSort
			();
		...
		private static void fix(org.apache.hadoop.util.IndexedSortable s, int p, int r)
		{
			if (s.compare(p, r) > 0)
			{
				s.swap(p, r);
		...
		protected internal static int getMaxDepth(int x)
		{
			if (x <= 0)
			{
				throw new System.ArgumentException("Undefined for " + x);
			}
			return (32 - int.numberOfLeadingZeros(x - 1)) << 2;
		}
		...
		private static void sortInternal(org.apache.hadoop.util.IndexedSortable s, int p, 
			int r, org.apache.hadoop.util.Progressable rep, int depth)
		{
			if (null != rep)
			{
				rep.progress();
