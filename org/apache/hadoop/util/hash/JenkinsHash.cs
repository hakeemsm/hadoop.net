...
namespace org.apache.hadoop.util.hash
{
	...
	{
		private static long INT_MASK = unchecked((long)(0x00000000ffffffffL));

		private static long BYTE_MASK = unchecked((long)(0x00000000000000ffL));

		private static org.apache.hadoop.util.hash.JenkinsHash _instance = new org.apache.hadoop.util.hash.JenkinsHash
			();
		...
		{
			return ((int.rotateLeft((int)(val & INT_MASK), pos)) & INT_MASK);
		}
		...
		public override int hash(byte[] key, int nbytes, int initval)
		{
			...
			a = b = c = (unchecked((long)(0x00000000deadbeefL)) + length + initval) & INT_MASK;
			int offset = 0;
		...
		public static void Main(string[] args)
		{
			...
			int value = 0;
			org.apache.hadoop.util.hash.JenkinsHash hash = new org.apache.hadoop.util.hash.JenkinsHash
				();
			for (int length = @in.read(bytes); length > 0; length = @in.read(bytes))
			{
				value = hash.hash(bytes, length, value);
			}
			System.Console.Out.WriteLine(System.Math.abs(value));
		}
	}
}
