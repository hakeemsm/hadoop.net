...
namespace org.apache.hadoop.fi
{
	...
	public class ProbabilityModel
	{
		...
		private static readonly org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory
			.getLog(Sharpen.Runtime.getClassForType(typeof(org.apache.hadoop.fi.ProbabilityModel
			)));

		internal const string FPROB_NAME = "fi.";

		private const string ALL_PROBABILITIES = FPROB_NAME + "*";

		private const float DEFAULT_PROB = 0.00f;

		private const float MAX_PROB = 1.00f;

		private static org.apache.hadoop.conf.Configuration conf = org.apache.hadoop.fi.FiConfig
			.getConfig();
		...
		static ProbabilityModel()
		{
			...
			conf.set(ALL_PROBABILITIES, Sharpen.Runtime.getProperty(ALL_PROBABILITIES, conf.get
				(ALL_PROBABILITIES, float.toString(DEFAULT_PROB))));
		...
		protected internal static float getProbability(string klass)
		{
			...
			string newValue = Sharpen.Runtime.getProperty(newProbName, conf.get(ALL_PROBABILITIES
				));
