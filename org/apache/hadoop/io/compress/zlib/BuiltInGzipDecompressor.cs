...
namespace org.apache.hadoop.io.compress.zlib
{
	...
	/// http://www.gzip.org/
	/// </summary>
	public class BuiltInGzipDecompressor : org.apache.hadoop.io.compress.Decompressor
	{
		private const int GZIP_MAGIC_ID = unchecked((int)(0x8b1f));

		private const int GZIP_DEFLATE_METHOD = 8;

		private const int GZIP_FLAGBIT_HEADER_CRC = unchecked((int)(0x02));

		private const int GZIP_FLAGBIT_EXTRA_FIELD = unchecked((int)(0x04));

		private const int GZIP_FLAGBIT_FILENAME = unchecked((int)(0x08));

		private const int GZIP_FLAGBIT_COMMENT = unchecked((int)(0x10));

		private const int GZIP_FLAGBITS_RESERVED = unchecked((int)(0xe0));

		private java.util.zip.Inflater inflater = new java.util.zip.Inflater(true);
		...
		private java.util.zip.Checksum crc = org.apache.hadoop.util.DataChecksum.newCrc32
			();
		...
		private org.apache.hadoop.io.compress.zlib.BuiltInGzipDecompressor.GzipStateLabel
			 state;
		...
		public BuiltInGzipDecompressor()
		{
			...
			state = org.apache.hadoop.io.compress.zlib.BuiltInGzipDecompressor.GzipStateLabel
				.HEADER_BASIC;
			crc.reset();
		}
		...
		public virtual bool needsInput()
		{
			lock (this)
			{
				if (state == org.apache.hadoop.io.compress.zlib.BuiltInGzipDecompressor.GzipStateLabel
					.DEFLATE_STREAM)
				{
					// most common case
				...
				// verify userBufLen <= 0
				return (state != org.apache.hadoop.io.compress.zlib.BuiltInGzipDecompressor.GzipStateLabel
					.FINISHED);
			}
		}
		...
		private void executeHeaderState()
		{
			...
			if (state == org.apache.hadoop.io.compress.zlib.BuiltInGzipDecompressor.GzipStateLabel
				.HEADER_BASIC)
			{
				int n = System.Math.min(userBufLen, 10 - localBufOff);
				...
				.HEADER_EXTRA_FIELD)
			{
				if (hasExtraField)
				{
					...
					{
						int n = System.Math.min(userBufLen, 2 - localBufOff);
						checkAndCopyBytesToLocal(n);
						if (localBufOff >= 2)
						{
					...
					{
						int n = System.Math.min(userBufLen, numExtraFieldBytesRemaining);
						checkAndSkipBytes(n);
						// modifies userBufLen, etc.
				...
				.HEADER_CRC)
			{
				if (hasHeaderCRC)
				{
					...
					if (localBufOff >= 2)
					{
						...
						{
							throw new System.IO.IOException("gzip header CRC failure");
						}
						localBufOff = 0;
						crc.reset();
						state = org.apache.hadoop.io.compress.zlib.BuiltInGzipDecompressor.GzipStateLabel
		...
		private void executeTrailerState()
		{
				...
				.TRAILER_CRC)
			{
				...
				int n = System.Math.min(userBufLen, 4 - localBufOff);
				copyBytesToLocal(n);
				if (localBufOff >= 4)
				{
					...
					{
						throw new System.IO.IOException("gzip stream CRC failure");
					}
					localBufOff = 0;
					crc.reset();
					state = org.apache.hadoop.io.compress.zlib.BuiltInGzipDecompressor.GzipStateLabel
				...
				.TRAILER_SIZE)
			{
				...
				if (localBufOff >= 4)
				{
					...
					{
						throw new System.IO.IOException("stored gzip size doesn't match decompressed size"
							);
					}
					localBufOff = 0;
					state = org.apache.hadoop.io.compress.zlib.BuiltInGzipDecompressor.GzipStateLabel
		...
		private void processBasicHeader()
		{
			...
			*/
			...
			{
				throw new System.IO.IOException("not a gzip file");
			}
			if (readUByte(localBuf, 2) != GZIP_DEFLATE_METHOD)
			{
				throw new System.IO.IOException("gzip data not compressed with deflate method");
			}
			int flg = readUByte(localBuf, 3);
			if ((flg & GZIP_FLAGBITS_RESERVED) != 0)
			{
				throw new System.IO.IOException("unknown gzip format (reserved flagbits set)");
			}
		...
		private void copyBytesToLocal(int len)
		{
			...
			userBufLen -= len;
			if (state == org.apache.hadoop.io.compress.zlib.BuiltInGzipDecompressor.GzipStateLabel
				.TRAILER_CRC || state == org.apache.hadoop.io.compress.zlib.BuiltInGzipDecompressor.GzipStateLabel
				.TRAILER_SIZE)
			{
