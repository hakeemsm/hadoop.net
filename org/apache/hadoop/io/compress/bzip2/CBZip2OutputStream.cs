...
namespace org.apache.hadoop.io.compress.bzip2
{
	...
	/// </p>
	/// </remarks>
	public class CBZip2OutputStream : java.io.OutputStream, org.apache.hadoop.io.compress.bzip2.BZip2Constants
	{
		...
		private static void hbMakeCodeLengths(byte[] len, int[] freq, org.apache.hadoop.io.compress.bzip2.CBZip2OutputStream.Data
		...
		private org.apache.hadoop.io.compress.bzip2.CBZip2OutputStream.Data data;
		...
		*/
		...
		/// <see cref="MIN_BLOCKSIZE"/>
		/// and
		/// <see cref="MAX_BLOCKSIZE"/>
		...
		{
			return (inputLength > 0) ? (int)System.Math.min((inputLength / 132000) + 1, 9) : 
				MAX_BLOCKSIZE;
		}
		...
		public CBZip2OutputStream(java.io.OutputStream @out, int blockSize)
			: base()
		{
			if (blockSize < 1)
			{
				throw new System.ArgumentException("blockSize(" + blockSize + ") < 1");
			}
			if (blockSize > 9)
			{
				throw new System.ArgumentException("blockSize(" + blockSize + ") > 9");
			}
		...
		public override void write(int b)
		{
			...
			{
				throw new System.IO.IOException("closed");
			}
		}
		...
		private void writeRun()
		{
			...
			if (lastShadow < this.allowableBlockSize)
			{
				...
				org.apache.hadoop.io.compress.bzip2.CBZip2OutputStream.Data dataShadow = this.data;
		...
		public override void close()
		{
			if (@out != null)
			{
				java.io.OutputStream outShadow = this.@out;
				try
				{
					finish();
					outShadow.close();
				...
				{
					org.apache.hadoop.io.IOUtils.closeStream(outShadow);
				}
			}
		}
		...
		private void init()
		{
			...
			// this.out.write('B');
			// this.out.write('Z');
			this.data = new org.apache.hadoop.io.compress.bzip2.CBZip2OutputStream.Data(this.
				blockSize100k);
			/*
		...
		private void initBlock()
		{
			...
			this.allowableBlockSize = (this.blockSize100k * org.apache.hadoop.io.compress.bzip2.BZip2Constants
				.baseBlockSize) - 20;
		}
		...
		public override void write(byte[] buf, int offs, int len)
		{
			...
			{
				throw new System.IndexOutOfRangeException("offs(" + offs + ") + len(" + len + ") > buf.length("
					 + buf.Length + ").");
			}
			if (this.@out == null)
			{
		...
		private int sendMTFValues1(int nGroups, int alphaSize)
		{
			...
			for (int iter = 0; iter < N_ITERS; iter++)
			{
				...
				for (int gs = 0; gs < this.nMTF; )
				{
					...
					* coding tables.
					*/
					int ge = System.Math.min(gs + G_SIZE - 1, nMTFShadow - 1);
					if (nGroups == N_GROUPS)
					{
		...
		/// </p>
		/// </remarks>
		private bool mainSimpleSort(org.apache.hadoop.io.compress.bzip2.CBZip2OutputStream.Data
			 dataShadow, int lo, int hi, int d)
		{
			int bigN = hi - lo + 1;
			if (bigN < 2)
			{
...
HP_break: ;
		...
		private void mainQSort3(org.apache.hadoop.io.compress.bzip2.CBZip2OutputStream.Data
			 dataShadow, int loSt, int hiSt, int dSt)
		{
		...
		private void mainSort()
		{
			...
			* for block.
			*/
			for (int i_1 = 0; i_1 < NUM_OVERSHOOT_BYTES; i_1++)
			{
				block[lastShadow + i_1 + 2] = block[(i_1 % (lastShadow + 1)) + 1];
			}
			for (int i_2 = lastShadow + NUM_OVERSHOOT_BYTES + 1; --i_2 >= 0; )
			{
				quadrant[i_2] = (char)0;
			}
		...
		private void randomiseBlock()
		{
			...
			for (int i_1 = 0; i_1 <= lastShadow; i_1 = j, j++)
			{
				if (rNToGo == 0)
				{
					rNToGo = (char)org.apache.hadoop.io.compress.bzip2.BZip2Constants.rNums[rTPos];
					if (++rTPos == 512)
					{
						rTPos = 0;
					}
		...
		private sealed class Data : object
		{
			...
			internal readonly int[] mtfFreq = new int[MAX_ALPHA_SIZE];
			...
			internal readonly byte[][] sendMTFValues_len = new byte[][] { new byte[MAX_ALPHA_SIZE
				], new byte[MAX_ALPHA_SIZE], new byte[MAX_ALPHA_SIZE], new byte[MAX_ALPHA_SIZE], 
				new byte[MAX_ALPHA_SIZE], new byte[MAX_ALPHA_SIZE] };

			internal readonly int[][] sendMTFValues_rfreq = new int[][] { new int[MAX_ALPHA_SIZE
				], new int[MAX_ALPHA_SIZE], new int[MAX_ALPHA_SIZE], new int[MAX_ALPHA_SIZE], new 
				int[MAX_ALPHA_SIZE], new int[MAX_ALPHA_SIZE] };

			internal readonly int[] sendMTFValues_fave = new int[N_GROUPS];

			internal readonly short[] sendMTFValues_cost = new short[N_GROUPS];

			internal readonly int[][] sendMTFValues_code = new int[][] { new int[MAX_ALPHA_SIZE
				], new int[MAX_ALPHA_SIZE], new int[MAX_ALPHA_SIZE], new int[MAX_ALPHA_SIZE], new 
				int[MAX_ALPHA_SIZE], new int[MAX_ALPHA_SIZE] };

			internal readonly byte[] sendMTFValues2_pos = new byte[N_GROUPS];

			internal readonly bool[] sentMTFValues4_inUse16 = new bool[16];

			internal readonly int[] stack_ll = new int[QSORT_STACK_SIZE];

			internal readonly int[] stack_hh = new int[QSORT_STACK_SIZE];

			internal readonly int[] stack_dd = new int[QSORT_STACK_SIZE];
			...
			internal readonly int[] heap = new int[MAX_ALPHA_SIZE + 2];

			internal readonly int[] weight = new int[MAX_ALPHA_SIZE * 2];

			internal readonly int[] parent = new int[MAX_ALPHA_SIZE * 2];
				...
				: base()
			{
				...
				// 8433529 byte
				// ============
				int n = blockSize100k * org.apache.hadoop.io.compress.bzip2.BZip2Constants.baseBlockSize;
