...
{
	public class TestFsPermission : NUnit.Framework.TestCase
	{
		public virtual void testFsAction()
		{
			//implies
			foreach (org.apache.hadoop.fs.permission.FsAction a in org.apache.hadoop.fs.permission.FsAction
				.values())
			{
				NUnit.Framework.Assert.IsTrue(org.apache.hadoop.fs.permission.FsAction.ALL.implies
					(a));
			}
			foreach (org.apache.hadoop.fs.permission.FsAction a_1 in org.apache.hadoop.fs.permission.FsAction
				.values())
			{
				NUnit.Framework.Assert.IsTrue(a_1 == org.apache.hadoop.fs.permission.FsAction.NONE
					 ? org.apache.hadoop.fs.permission.FsAction.NONE.implies(a_1) : !org.apache.hadoop.fs.permission.FsAction
					.NONE.implies(a_1));
			}
			foreach (org.apache.hadoop.fs.permission.FsAction a_2 in org.apache.hadoop.fs.permission.FsAction
				.values())
			{
				NUnit.Framework.Assert.IsTrue(a_2 == org.apache.hadoop.fs.permission.FsAction.READ_EXECUTE
					 || a_2 == org.apache.hadoop.fs.permission.FsAction.READ || a_2 == org.apache.hadoop.fs.permission.FsAction
					.EXECUTE || a_2 == org.apache.hadoop.fs.permission.FsAction.NONE ? org.apache.hadoop.fs.permission.FsAction
					.READ_EXECUTE.implies(a_2) : !org.apache.hadoop.fs.permission.FsAction.READ_EXECUTE
					.implies(a_2));
			}
			//masks
			NUnit.Framework.Assert.AreEqual(org.apache.hadoop.fs.permission.FsAction.EXECUTE, 
				org.apache.hadoop.fs.permission.FsAction.EXECUTE.and(org.apache.hadoop.fs.permission.FsAction
				.READ_EXECUTE));
			NUnit.Framework.Assert.AreEqual(org.apache.hadoop.fs.permission.FsAction.READ, org.apache.hadoop.fs.permission.FsAction
				.READ.and(org.apache.hadoop.fs.permission.FsAction.READ_EXECUTE));
			NUnit.Framework.Assert.AreEqual(org.apache.hadoop.fs.permission.FsAction.NONE, org.apache.hadoop.fs.permission.FsAction
				.WRITE.and(org.apache.hadoop.fs.permission.FsAction.READ_EXECUTE));
			NUnit.Framework.Assert.AreEqual(org.apache.hadoop.fs.permission.FsAction.READ, org.apache.hadoop.fs.permission.FsAction
				.READ_EXECUTE.and(org.apache.hadoop.fs.permission.FsAction.READ_WRITE));
			NUnit.Framework.Assert.AreEqual(org.apache.hadoop.fs.permission.FsAction.NONE, org.apache.hadoop.fs.permission.FsAction
				.READ_EXECUTE.and(org.apache.hadoop.fs.permission.FsAction.WRITE));
			NUnit.Framework.Assert.AreEqual(org.apache.hadoop.fs.permission.FsAction.WRITE_EXECUTE
				, org.apache.hadoop.fs.permission.FsAction.ALL.and(org.apache.hadoop.fs.permission.FsAction
				.WRITE_EXECUTE));
		}
		...
		public virtual void testConvertingPermissions()
		{
			...
			{
				NUnit.Framework.Assert.AreEqual(s, new org.apache.hadoop.fs.permission.FsPermission
					(s).toShort());
			}
			short s_1 = 0;
			foreach (bool sb in new bool[] { false, true })
			{
				foreach (org.apache.hadoop.fs.permission.FsAction u in org.apache.hadoop.fs.permission.FsAction
					.values())
				{
					foreach (org.apache.hadoop.fs.permission.FsAction g in org.apache.hadoop.fs.permission.FsAction
						.values())
					{
						foreach (org.apache.hadoop.fs.permission.FsAction o in org.apache.hadoop.fs.permission.FsAction
							.values())
						{
							// Cover constructor with sticky bit.
							org.apache.hadoop.fs.permission.FsPermission f = new org.apache.hadoop.fs.permission.FsPermission
								(u, g, o, sb);
							NUnit.Framework.Assert.AreEqual(s_1, f.toShort());
							org.apache.hadoop.fs.permission.FsPermission f2 = new org.apache.hadoop.fs.permission.FsPermission
								(f);
					...
					.values())
				{
						...
						.values())
					{
							...
							.values())
						{
							...
							if (f.getStickyBit() && f.getOtherAction().implies(org.apache.hadoop.fs.permission.FsAction
								.EXECUTE))
							{
		...
		public virtual void testFsPermission()
		{
			...
			for (int i = 0; i < (1 << 10); i++)
			{
				...
				NUnit.Framework.Assert.AreEqual(i, org.apache.hadoop.fs.permission.FsPermission.valueOf
					(b.ToString()).toShort());
			}
		}
				...
				.values())
			{
					...
					.values())
				{
						...
						.values())
					{
						...
						conf.set(org.apache.hadoop.fs.permission.FsPermission.UMASK_LABEL, asOctal);
						org.apache.hadoop.fs.permission.FsPermission fromConf = org.apache.hadoop.fs.permission.FsPermission
							.getUMask(conf);
		...
		public virtual void testSymbolicUmasks()
		{
			...
			{
				conf.set(org.apache.hadoop.fs.permission.FsPermission.UMASK_LABEL, symbolic[i][0]
					);
				short val = short.valueOf(symbolic[i][1], 8);
				NUnit.Framework.Assert.AreEqual(val, org.apache.hadoop.fs.permission.FsPermission
					.getUMask(conf).toShort());
			}
		}
		...
		public virtual void testBadUmasks()
		{
			...
			{
				conf.set(org.apache.hadoop.fs.permission.FsPermission.UMASK_LABEL, b);
				try
				{
					org.apache.hadoop.fs.permission.FsPermission.getUMask(conf);
		...
		public virtual void testDeprecatedUmask()
		{
			org.apache.hadoop.conf.Configuration conf = new org.apache.hadoop.conf.Configuration
				();
			conf.set(org.apache.hadoop.fs.permission.FsPermission.DEPRECATED_UMASK_LABEL, "302"
				);
			// 302 = 0456
